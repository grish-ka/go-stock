...
if [ ! -d "$CONFIG_DIR" ]; then
    MSG="Welcome to your first run of Go-Stock!"
else
    MSG="Go-Stock has been updated successfully."
fi

# 6. Direct-to-TTY Output
{
    printf "${B}%b${NC}  ${G}Go-Stock identified: %s${NC}\n" "$ICON" "$OS_NAME"
    [ "$IS_WSL" = true ] && printf "${C}(Running on Windows 11 WSL)${NC}\n"
    printf "${G}%s${NC}\n" "$MSG"
    printf "Type ${B}go-stock${NC} to begin.\n"
